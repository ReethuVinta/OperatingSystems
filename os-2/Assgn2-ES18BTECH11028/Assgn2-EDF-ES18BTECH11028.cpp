...
void edf_scheduling(process P[])        // Function that performs edf_scheduling on the given list of processes
{
    ...
    log_ptr=freopen("EDF-Log.txt","w+",stdout);   // Creating a new txt file to write the outputs      
    if(log_ptr==NULL)     // If there is error while open the file then this block gets executed
    {
        printf("Couldn't open ED-Log.txt file \n");
        exit(1);                        
    }
    ...
    while (!event_queue.empty() || !ready_queue.empty())   // Keep running the loop until at least one of the queues is not empty
    {
        ...
        if(!ready_queue.empty())    // If there are processes already in ready queue that are to scheduled
        {
            ...
            if(P[ProcessID-1].RemainingBurstTime == P[ProcessID-1].Bursttime)  // If the remaining burst time is full then we immediatly schedule this process as this was not yet scheduled till now
            {
            ...
            if(event_queue.empty())     // If there are no elements present in event queue, that can be sent inro ready queue next
            {
            ...
            else // If there are elements present in the event queue that are ready to be pushed into ready queue
            {
                ...
                else if(event_queue.top().second < P[ProcessID-1].NextDeadline)
                {
                    ...
                    if(P[ProcessID-1].NextDeadline > P[event_queue.top().first-1].NextDeadline) // Checking the period, to see the priority
                    {
                        cout<<"6 Process P"<<ProcessID<<" is prempted by Process P"<<event_queue.top().first<<" at time "<<current_time;
...
int main()
{
    ...
    stats_ptr=freopen("EDF-Stats.txt","w+",stdout);  // Creating a new txt file to write the outputs      
    if(stats_ptr==NULL)                     // If there is error while open the file then this block gets executed
    {
        printf("Couldn't open EDF-Stats.txt file \n");
        exit(1);                        
    }
