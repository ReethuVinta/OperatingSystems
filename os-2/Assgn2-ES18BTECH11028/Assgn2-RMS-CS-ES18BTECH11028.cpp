...
void rms_scheduling(process P[])        // Function that performs rms_scheduling on the given list of processes
{
    ...
    log_ptr=freopen("RMS-CS-Log.txt","w+",stdout);    // Creating a new txt file to write the outputs      
    if(log_ptr==NULL)     // If there is error while open the file then this block gets executed
    {
        printf("Couldn't open RMS-CS-Log.txt file \n");
        exit(1);                        
    }
    ...
    { 
        if(!event_queue.empty() && abs(event_queue.top().second-current_time)<1e-4) // If the top element in the event queue is equal to the current time
        {
            ...
            {
                if(P[ProcessID-1].RemainingBurstTime != 0) // The process entered the event_queue but couldn't finish the task
                {
        ...
        if(!ready_queue.empty())   // If there are processes already in ready queue that are to be scheduled
        {
            ...
            if(abs(P[ProcessID-1].RemainingBurstTime-P[ProcessID-1].Bursttime)<1e-4) // If the remaining burst time is full then we immediatly schedule this process
            {
                ...
                current_time+=context_switch; // Adding context switch
...
int main()
{
    ...
    stats_ptr=freopen("RMS-CS-Stats.txt","w+",stdout);  // Creating a new txt file to write the outputs      
    if(stats_ptr==NULL)                     // If there is error while open the file then this block gets executed
    {
        printf("Couldn't open RMS-CS-Stats.txt file \n");
        exit(1);                        
    }
